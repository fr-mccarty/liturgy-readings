    ...
    <x-form-section submit="save">
        ...
        <x-slot name="form">
            @if(empty($liturgy['collectNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.collectId">
                        ...
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'collect')->where('translation_id', '1') as $prayer)
                            <option value="{{$prayer['id']}}">@if(!empty($prayer['for'])) For: {{$prayer['for']}} -  @endif {{$prayer['text']}}</option>
                        @endforeach
                    </x-input.select>
            ...
            @if(empty($liturgy['offeringsNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.offeringsId">
                        <option value="">Select</option>
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'offerings')->where('translation_id', '1') as $prayer)
            ...
            @if(empty($liturgy['prefaceNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.prefaceId">
                        <option value="">Select</option>
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'preface')->where('translation_id', '1') as $prayer)
                            <option value="{{$prayer['id']}}">{{$prayer['text']}}</option>
                        @endforeach
                    </x-input.select>
            ...
            @if(empty($liturgy['afterCommunionNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.afterCommunionId">
                        <option value="">Select</option>
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'after_communion')->where('translation_id', '1') as $prayer)
                            <option value="{{$prayer['id']}}">{{$prayer['text']}}</option>
                        @endforeach
                    </x-input.select>
            ...
            @if(empty($liturgy['invitationToPrayerNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    <x-label>Invitation to Prayer</x-label>
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.invitationToPrayerId">
                        <option value="">Select</option>
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'invitation_to_prayer')->where('translation_id', '1') as $prayer)
                            <option value="{{$prayer['id']}}">{{$prayer['text']}}</option>
                        @endforeach
                    </x-input.select>
                    @error('liturgy.invitationToPrayerId')
                    <x-input.error>{{ $message }}</x-input.error>
                    @enderror
                </div>
            @endif
            ...
            <div class="col-span-6 sm:col-span-4">
                <div class="flex space-x-2 items-center">
                    <x-label>Do not Print</x-label>
                    <x-input.checkbox wire:model="liturgy.invitationToPrayerNoPrint"></x-input.checkbox>
                </div>
                <hr class="mt-3">
            </div>
            ...
            @if(empty($liturgy['finalCommendationNoPrint']))
                <div class="col-span-6 sm:col-span-4">
                    <x-label>Final Commendation</x-label>
                    <x-input.select class="mt-1" wire:model.lazy="liturgy.finalCommendationId">
                        <option value="">Select</option>
                        @foreach($prayers->where('liturgy_type', 'funeral')->where('type', 'final_commendation')->where('translation_id', '1') as $prayer)
                            <option value="{{$prayer['id']}}">{{$prayer['text']}}</option>
                        @endforeach
                    </x-input.select>
                    @error('liturgy.finalCommendationId')
                    <x-input.error>{{ $message }}</x-input.error>
                    @enderror
                </div>
            @endif
            ...
            <div class="col-span-6 sm:col-span-4">
                <div class="flex space-x-2 items-center">
                    <x-label>Do not Print</x-label>
                    <x-input.checkbox wire:model="liturgy.finalCommendationNoPrint"></x-input.checkbox>
                </div>
                <hr class="mt-3">
            </div>
